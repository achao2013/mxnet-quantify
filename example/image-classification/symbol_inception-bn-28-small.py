...
def ConvFactory(data, num_filter, kernel, stride=(1,1), pad=(0, 0), act_type="relu", mirror_attr={}):
    ...
    return act
# Binary Conv + BN + ReLU factory
def ConvBinaryFactory(data, num_filter, kernel, stride=(1,1), pad=(0, 0), act_type="relu", mirror_attr={}):
    conv = mx.symbol.Convolution(data=data, num_filter=num_filter, kernel=kernel, stride=stride, pad=pad, w_project_method=9)
    bn = mx.symbol.BatchNorm(data=conv)
    act = mx.symbol.Activation(data = bn, act_type=act_type, attr=mirror_attr)
    return act
...
def SimpleFactory(data, ch_1x1, ch_3x3, mirror_attr):
    ...
    return concat
def SimpleBinaryFactory(data, ch_1x1, ch_3x3, mirror_attr):
    # 1x1
    conv1x1 = ConvBinaryFactory(data=data, kernel=(1, 1), pad=(0, 0), num_filter=ch_1x1, mirror_attr=mirror_attr)
    # 3x3
    conv3x3 = ConvBinaryFactory(data=data, kernel=(3, 3), pad=(1, 1), num_filter=ch_3x3, mirror_attr=mirror_attr)
    #concat
    concat = mx.symbol.Concat(*[conv1x1, conv3x3])
    return concat
def get_symbol(num_classes = 10, force_mirroring=False):
    ...
    in3a = SimpleBinaryFactory(conv1, 32, 32, mirror_attr=attr)
    in3b = SimpleBinaryFactory(in3a, 32, 48, mirror_attr=attr)
    in3c = DownsampleFactory(in3b, 80, mirror_attr=attr)
    in4a = SimpleBinaryFactory(in3c, 112, 48, mirror_attr=attr)
    in4b = SimpleBinaryFactory(in4a, 96, 64, mirror_attr=attr)
    in4c = SimpleBinaryFactory(in4b, 80, 80, mirror_attr=attr)
    in4d = SimpleBinaryFactory(in4c, 48, 96, mirror_attr=attr)
    in4e = DownsampleFactory(in4d, 96, mirror_attr=attr)
    in5a = SimpleBinaryFactory(in4e, 176, 160, mirror_attr=attr)
    in5b = SimpleBinaryFactory(in5a, 176, 160, mirror_attr=attr)
