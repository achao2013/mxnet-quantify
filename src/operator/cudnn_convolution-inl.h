...
class CuDNNConvolutionOp : public Operator {
 public:
  ...
  //add by jzc
   ...
   {
	  //LOG(INFO)<<"tmp_w ctx:"<<tmp_w.dev_mask_;
 	  projected_weight = tmp_w;
 	 //LOG(INFO)<<"cudnn conv w_project_method:"<<get_project_method();
   }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    //add by jzc
    //LOG(INFO)<<"projected_weight.get start in cudnn conv";
    //LOG(INFO)<<"projected_weight ctx:"<<projected_weight.dev_mask_;
    Tensor<gpu, 4, DType> pwmat =projected_weight.get<gpu, 4, DType>(s);
    //LOG(INFO)<<"cudnn convolution forward";
    //Tensor<gpu, 4, DType> pwmat=mshadow::NewTensor<gpu,DType,4>(wmat.shape_,DType(0),false,s);
    //LOG(INFO)<<"projected_weight.get end in cudnn";
    CHECK_EQ(pwmat.CheckContiguous(), true);
	  ...
	  switch(this->param_.w_project_method)
	  {
		  case conv::None:
			  CHECK_EQ(cudnnConvolutionForward(s->dnn_handle_,
													 ...
													 &beta,
													 out_desc_,
													 out.dptr_ + out_offset_ * g), CUDNN_STATUS_SUCCESS);//original
			  //LOG(INFO)<<"cudnn convolution forward of none";
			break;
		  case conv::Binary:
		  {
			//pwmat=mshadow::expr::F<mshadow_op::binary>(wmat);
			Assign(pwmat,kWriteTo,mshadow::expr::F<mshadow_op::binary>(wmat));
			/*print_check<gpu>(pwmat.dptr_);
			Tensor<cpu, 4, DType> cpu_pwmat=mshadow::NewTensor<cpu,DType, 4>(pwmat.shape_,DType(0),false);
			Copy(cpu_pwmat, pwmat,s);
			std::cout<<"forward pwmat"<<cpu_pwmat.dptr_[0]<<" "<<cpu_pwmat.dptr_[1]<<" "<<cpu_pwmat.dptr_[2];*/
			//LOG(INFO)<<"cudnn convolution forward of binary";
			CHECK_EQ(cudnnConvolutionForward(s->dnn_handle_,
												   &alpha,
												   in_desc_,
												   data.dptr_ + data_offset_ * g,
												   filter_desc_,
												   pwmat.dptr_ + weight_offset_ * g,
												   conv_desc_,
												   algo_,
												   workspace.dptr_,
												   forward_workspace_byte_,
			...
			break;
		  case conv::SimpleScaleBinary:
				mx_xpu_asum<gpu,DType>(s, wmat.shape_.Size(), wmat.dptr_, &tmp);
				tmp=tmp/wmat.shape_.Size();
				//pwmat=mshadow::expr::F<mshadow_op::binary>(wmat);
				Assign(pwmat,kWriteTo,mshadow::expr::F<mshadow_op::binary>(wmat));
				pwmat=pwmat*mshadow::expr::scalar<DType>(tmp);
				CHECK_EQ(cudnnConvolutionForward(s->dnn_handle_,
													   &alpha,
													   in_desc_,
													   data.dptr_ + data_offset_ * g,
		  ...
		  case conv::Sign:
			break;
		  case conv::FourBit:
		     //pwmat=mshadow::expr::F<mshadow_op::fourbit>(wmat);
		     Assign(pwmat,kWriteTo,mshadow::expr::F<mshadow_op::fourbit>(wmat));
		     //LOG(INFO)<<"cudnn convolution forward of fourbit";
		     CHECK_EQ(cudnnConvolutionForward(s->dnn_handle_,
		     												   &alpha,
		     												   in_desc_,
		     												   data.dptr_ + data_offset_ * g,
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    for (uint32_t g = 0; g < param_.num_group; ++g) {
      ...
      CHECK_EQ(cudnnConvolutionBackwardFilter_v3(s->dnn_handle_,
              		               ...
              		               filter_desc_,
              		               gwmat.dptr_ + weight_offset_ * g), CUDNN_STATUS_SUCCESS);//original
#elif CUDNN_MAJOR == 5
      ...
      case conv::None:
    	  break;
      case conv::Binary:
    	  Assign(gwmat, kWriteInplace, mshadow::expr::F<mshadow_op::binary_grad>(wmat)*gwmat);
    	  break;
      }

      /*
            Tensor<gpu, 4, DType> tmp_gwmat=mshadow::NewTensor(gwmat.shape_,DType(0),true,s);
            #if CUDNN_MAJOR <= 4
            switch(this->param_.w_project_method)
            	  {
            	  case conv::None:
            	  ...
            	  case conv::Binary:
            		CHECK_EQ(cudnnConvolutionBackwardFilter_v3(s->dnn_handle_,
            		               ...
            		               &beta,//edit by jzc
            		               filter_desc_,
            		               tmp_gwmat.dptr_ + weight_offset_ * g), CUDNN_STATUS_SUCCESS);
            		break;
            	  }
	  ...
	  case conv::None:
		  CHECK_EQ(cudnnConvolutionBackwardData_v3(s->dnn_handle_,
						 ...
						 &beta,
						 in_desc_,
						 gdata.dptr_ + data_offset_ * g), CUDNN_STATUS_SUCCESS);//original
		  //LOG(INFO)<<"cudnn convolution backward of none";
		break;
	  case conv::Binary:
		//pwmat=mshadow::expr::F<mshadow_op::binary>(wmat);
		Assign(pwmat,kWriteTo,mshadow::expr::F<mshadow_op::binary>(wmat));
		//print_check<gpu>(pwmat.dptr_);
		//LOG(INFO)<<"cudnn convolution backward of binary";
		CHECK_EQ(cudnnConvolutionBackwardData_v3(s->dnn_handle_,
					   &alpha,
					   filter_desc_,
					   pwmat.dptr_ + weight_offset_ * g,
	  ...
	  case conv::FourBit:
		  Assign(pwmat,kWriteTo,mshadow::expr::F<mshadow_op::fourbit>(wmat));
		 // LOG(INFO)<<"cudnn convolution backward of fourbit";
		  CHECK_EQ(cudnnConvolutionBackwardData_v3(s->dnn_handle_,
		  					   &alpha,
		  					   filter_desc_,
		  					   pwmat.dptr_ + weight_offset_ * g,
    ...
    //FreeSpace<4,DType>(&pwmat);//memory fragment
  }
  ...
  inline void Init(mshadow::Stream<gpu> *s,
    ...
    if (!init_cudnn_) {
      ...
      switch(this->param_.w_project_method)
      	  ...
      	  case conv::Binary:
      	  case conv::FourBit:
      		forward_workspace_+=param_.num_filter  * data.shape_[1] / param_.num_group* param_.kernel[0] * param_.kernel[1];
      		backward_workspace_+=param_.num_filter  * data.shape_[1] / param_.num_group* param_.kernel[0] * param_.kernel[1];
      		break;
      	  }*/
    }
  }
