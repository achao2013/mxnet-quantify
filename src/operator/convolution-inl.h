...
//add by jzc
enum ConvolutionOpProjectWeights {None,Binary,SimpleScaleBinary,Sign,Round,Power,Stoch,StochM,AddMorm,MultUnif,FourBit};
}
...
struct ConvolutionParam : public dmlc::Parameter<ConvolutionParam> {
  ...
  int cudnn_tune;
  //edit by jzc
  typedef conv::ConvolutionOpProjectWeights OpProjectWeights;
  ...
  DMLC_DECLARE_PARAMETER(ConvolutionParam) {
    ...
    DMLC_DECLARE_FIELD(w_project_method).set_default(conv::None)
...
class ConvolutionOp : public Operator {
 public:
  ...
  virtual void init_projection(TBlob& tmp_w)
  {
	  ...
	  //LOG(INFO)<<"conv w_project_method:"<<get_project_method();
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    //add by jzc
    //LOG(INFO)<<"projected_weight.get start in conv";
    //LOG(INFO)<<"projected_weight ctx:"<<projected_weight.dev_mask_;
    Tensor<xpu, 3, DType> pwmat =projected_weight.get_with_shape<xpu, 3, DType>(wmat_shape, s);
    //LOG(INFO)<<"convolution forward";
    //Tensor<xpu, 3, DType> pwmat=mshadow::NewTensor<xpu,DType,3>(wmat.shape_,DType(0),true,s);
    ...
    for (index_t i = 0; i < nbatch; i += nstep_) {
      ...
      for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
        ...
        switch(this->param_.w_project_method)
            {
            case conv::None:
            	//LOG(INFO)<<"convolution forward of none";
            	temp_dst[gid] = dot(wmat[gid], tmpc);//original
            	break;
            case conv::Binary:
            	pwmat=F<mshadow_op::binary>(wmat);
            	//LOG(INFO)<<"convolution forward of binary";
            	temp_dst[gid] = dot(pwmat[gid], tmpc);
            	break;
            ...
            case conv::FourBit:
                ...
                LOG(INFO)<<" convolution forward of fourbit";
                temp_dst[gid] = dot(pwmat[gid], tmpc);
                break;
            }
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    for (index_t i = 0; i < nbatch; i += nstep_) {
      ...
      case conv::None:
    	  break;
      case conv::Binary:
    	  //Assign(gwmat, kWriteInplace, F<mshadow_op::clip5>(gwmat));
    	  break;
      }
      for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
        Tensor<xpu, 2, DType> tmpc = temp_col.Slice(gstride * gid, gstride * (gid + 1));
        ...
        //add by jzc
                switch(this->param_.w_project_method)
                    {
                    case conv::None:
                    	//LOG(INFO)<<"convolution backward of none";
                    	tmpc = dot(wmat[gid].T(), temp_dst[gid]);//original
                    	break;
                    case conv::Binary:
                    	pwmat=F<mshadow_op::binary>(wmat);
                    	//LOG(INFO)<<"convolution backward of binary";
                    	tmpc = dot(pwmat[gid].T(), temp_dst[gid]);
                    	break;
                    case conv::Sign:
                    	break;
                    case conv::FourBit:
                    	pwmat=F<mshadow_op::fourbit>(wmat);
                    	//LOG(INFO)<<"convolution backward of fourbit";
                    	tmpc = dot(pwmat[gid].T(), temp_dst[gid]);
                    }
      }
      if (param_.pad[0] == 0 && param_.pad[1] == 0) {
 ...
 private:
  inline index_t InitTemp(const mshadow::Shape<4> &ishape,
   ...
   /* switch(this->param_.w_project_method)
          	  ...
          	  case conv::Binary:
          	  case conv::FourBit:
          		required_size+=param_.num_filter  * ishape[1] / param_.num_group* param_.kernel[0] * param_.kernel[1];
          		break;
          	  }*/
