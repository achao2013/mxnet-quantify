...
class StaticGraph {
 public:
  ...
  //add by jzc
  inline std::string get_op_type(int nid)
  {
	  if (nodes[nid].is_variable())
		  	return "";
	  else if(nodes[nid].is_forward())
		    return nodes[nid].op->TypeString();
	  else if(nodes[nid].is_backward())
	  		return nodes[nodes[nid].backward_source_id].op->TypeString();
  }
  /*!
   * \brief interface for json serialization.
   * \param writer the JSON writer write json.
