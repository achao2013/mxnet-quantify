...
GraphStorageAllocator::Alloc(Context ctx, int type_flag, size_t size) {
  ...
  ptr->max_size = size;
  data_.push_back(std::move(ptr));
  LOG(INFO) <<"alloc id:"<<id<<" max_size:"<<size;
  return id;
}
...
GraphStorageAllocator::Request(Context ctx, int type_flag, TShape shape, uint32_t node_id) {
  ...
  for (auto it = mid; it != end; ++it) {
    ...
    free_.erase(it);
    LOG(INFO) <<"node_id:"<<node_id<<" e->id:"<<e->id<<" e->max_size:"<<e->max_size;
    return e->id;
  }
...
//add by jzc
size_t GraphStorageAllocator::InitProjectedWeightStorage(std::shared_ptr<NDArray> &tmp)
{
	StorageEntry *e = data_[data_.size()-1].get();
	CHECK_EQ(e->id, data_.size()-1);
	LOG(INFO)<<"id:"<<e->id<<" max_size:"<<e->max_size<<" type_flag:"<<e->type_flag;

	TShape shape = mshadow::Shape1(e->max_size);

	//e->data=NDArray(shape, e->ctx, false, e->type_flag);
	//tmp=std::shared_ptr<NDArray>(&e->data);

	tmp=std::shared_ptr<NDArray>(new NDArray(shape, e->ctx, false, e->type_flag));
	e->data=*(tmp.get());

	size_t total = e->max_size * mshadow::mshadow_sizeof(e->type_flag);
	shared_mem_->pool.push_back(e->data);
	LOG(INFO) <<"pool size:"<<shared_mem_->pool.size()<<" data_ size:"<<data_.size();
	CHECK_EQ(shared_mem_->pool.size(), data_.size());
	return total;
}
}  // namespace mxnet
