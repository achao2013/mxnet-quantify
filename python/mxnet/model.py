    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                for i, x in enumerate(zip(executor_manager.aux_names, executor_manager.aux_arrays)): #add by jzc
                    name,block=x                  
                    if name.endswith('nbatch'):                       
                        #print executor_manager.aux_arrays[i][0].shape
                        aux_params[name][0]=nbatch
                        #print aux_params[name].shape
                        #print executor_manager.aux_arrays[i][0].shape
                        
                        #aux_params[name].copyto(executor_manager.aux_arrays[i][0])
                executor_manager.set_params(arg_params, aux_params)
