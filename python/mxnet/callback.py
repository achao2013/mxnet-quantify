...
class debug_info(object):
    ...
    def __call__(self,param):
        count = param.nbatch
        if count % self.frequent == 0:
            param_arrays = param.em.param_arrays
            grad_arrays = param.em.grad_arrays
            param_names = param.em.param_names
            #aux_params = param.locals['aux_params']
            #for k,v in aux_params.items():
                #logging.info("param name :[%s]",k)
            for i in range(len(param_names)):               
                logging.info("param name :[%s],  mean of param:[%f], mean of grad:[%f], max of grad:[%f], min of grad:[%f]",
                param_names[i],np.mean(param_arrays[i][0].asnumpy()),np.mean(grad_arrays[i][0].asnumpy()),np.max(grad_arrays[i][0].asnumpy()),np.min(grad_arrays[i][0].asnumpy()))
