...
from .ndarray import NDArray, zeros, clip, sqrt, array
...
class Optimizer(object):
    ...
    def __init__(self, rescale_grad=1., param_idx2name=None, wd=0.,
                 ...
                 clip_weight=None, noclip_weight_layer=None,
                 binary_regula=None,no_binary_regula_layer=None,
        ...
        self.noclip_weight_layer = noclip_weight_layer
        self.binary_regula = binary_regula
        self.no_binary_regula_layer = no_binary_regula_layer
...
class SGD(Optimizer):
    ...
    def update(self, index, weight, grad, state):
        ...
        if self.clip_weight is not None:
                noclip_list = [int(n) for n in self.noclip_weight_layer.split(',')]
                #if index not in  noclip_list:
                    #print np.mean(weight.asnumpy())
                if self.idx2name[index].endswith('_weight'):
                    #print self.clip_weight
                    weight[:] = clip(weight, -self.clip_weight, self.clip_weight)
...
class Adam(Optimizer):
    ...
    def update(self, index, weight, grad, state):
        ...
        if wd > 0. and (self.binary_regula is None):
            weight[:] -= (lr * wd) * weight
        elif wd > 0. and (self.binary_regula == 'B_L2'):
            no_regula_list = [int(n) for n in self.no_binary_regula_layer.split(',')]
            if index not in  no_regula_list:
                weight.wait_to_read()
                tmp_weight=weight.asnumpy()
                weight_sign=weight.copy()
                weight_sign.wait_to_read()                
                np.where(tmp_weight>0,1,np.where(tmp_weight<0,-1,0))
                x=array(tmp_weight,weight_sign.context)
                x.copyto(weight_sign)
                weight_sign.wait_to_read()
                weight[:] -= (lr * wd) * (weight-weight_sign)
            else:
                weight[:] -= (lr * wd) * weight
